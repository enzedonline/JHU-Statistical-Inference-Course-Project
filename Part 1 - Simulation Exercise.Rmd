...
```{r run_simulations}
set.seed(123456)
mean_sim_rows <- rowMeans(matrix(rexp(n = count_sims * n, rate = lambda), count_sims, n))  
```
...
```{r meadian_skew}
sim_median <- median(mean_sim_rows); sim_skew <- skewness(mean_sim_rows)
```
...
ggplot(df_plot, aes(sample = mean_sim_rows)) +
    theme_minimal() + ggtitle ("Normal Q-Q Plot") + 
